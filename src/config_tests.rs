...
fn declaring(patterns: &[&str]) -> ModuleMetadata {
    ModuleMetadata {
        match_window_class: Some(patterns.iter().map(|s| s.to_string()).collect()),
        ..Default::default()
    }
}

#[test]
fn a_bare_name_matches_the_publisher_prefixed_class() {
    // The case that broke: Claude Desktop 2.x reports com.anthropic.Claude,
    // and the config had been maintaining one entry per packaging.
    let m = declaring(&["claude"]);
    assert!(m.matches_window_class("com.anthropic.Claude"));
    assert!(m.matches_window_class("Claude"));
    assert!(m.matches_window_class("claude"));
}

#[test]
fn one_entry_covers_both_spellings_every_config_used_to_list() {
    let m = declaring(&["firefox"]);
    assert!(m.matches_window_class("firefox"));
    assert!(m.matches_window_class("org.mozilla.firefox"));
}

#[test]
fn capitalisation_never_decides() {
    // X11 and Wayland disagree about it for the same application.
    let m = declaring(&["Konsole"]);
    assert!(m.matches_window_class("org.kde.konsole"));
    assert!(m.matches_window_class("KONSOLE"));
}

#[test]
fn a_fragment_of_a_name_is_not_a_match() {
    // "claude" must not claim the terminal window of a container named
    // claude-box. Whole names only — this is the line a substring rule crosses.
    let m = declaring(&["claude"]);
    assert!(!m.matches_window_class("claude-box"));
    assert!(!m.matches_window_class("claude-desktop"));
    assert!(!m.matches_window_class("io.github.aaddrick.claude-desktop-debian"));
}

#[test]
fn a_longer_name_is_not_matched_by_its_own_prefix_either() {
    let m = declaring(&["claude-desktop"]);
    assert!(m.matches_window_class("claude-desktop"));
    assert!(!m.matches_window_class("claude"));
}

#[test]
fn a_module_declaring_nothing_matches_nothing() {
    assert!(!ModuleMetadata::default().matches_window_class("firefox"));
}

#[derive(serde::Deserialize)]
struct ModuleWrapper { module: RawModuleMetadata }

#[test]
fn match_window_class_accepts_one_name_or_a_list() {
    // The doc comment promised both long before the parser did, and a bare
    // string failed the whole file with "expected a sequence".
    let one: ModuleWrapper =
        toml::from_str("[module]\nmatch_window_class = \"firefox\"\n").unwrap();
    let many: ModuleWrapper =
        toml::from_str("[module]\nmatch_window_class = [\"firefox\"]\n").unwrap();

    let one  = one.module.match_window_class.map(WindowClasses::into_vec);
    let many = many.module.match_window_class.map(WindowClasses::into_vec);
    assert_eq!(one, Some(vec!["firefox".to_string()]));
    assert_eq!(one, many);
}
