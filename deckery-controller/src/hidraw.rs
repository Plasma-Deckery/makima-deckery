...
//!
//!   When `event_tx` is supplied the reader also forwards each report to
//!   [`super::hid_report::synthesise_input_events`], which synthesises a full
...
//! ## Byte offsets
//!
//! `PadFrame`'s offsets were determined empirically (2026-07-08) by recording
//! evdev and hidraw simultaneously with a shared monotonic clock and
//! correlating ABS_HAT0/1X/Y against every int16 offset in the raw report.
//! Offsets match exactly (median deviation ~19 out of a ~65500 range,
//! explained by ~4ms sampling-phase jitter) and hidraw consistently leads
//! evdev by about one report (~4ms) — evidence that the evdev path goes
//! through an extra translation layer in the hid-steam kernel driver.
//!
//! The full button and axis map lives in [`super::hid_report`] and was
//! cross-checked against the upstream hid-steam kernel driver
//! (`steam_deck_button_mappings` / `steam_deck_axis_mappings` /
//! `steam_do_deck_input_event`).

use super::haptic::{HapticCommand, HapticRequest};
#[cfg(test)]
use super::haptic::HapticPad;
use super::hid_report::synthesise_input_events;
